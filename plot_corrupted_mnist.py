...
pathname = os.path.join('results',data_folder_name,'cnn_small')
...
def predictive_metrics_file(
    ...
    return os.path.join(seed_folder,
            f'PredictiveMetrics_{p_method}_Psi{psi_ref}_c-{corruption}.pt')
...
def kl_file(
    ...
    return os.path.join(seed_folder,
            f'kl_{p_method}_Psi{psi_ref}_c-{corruption}.pt')

def plot_file(metric: Literal['rel_error', 'trace', 'nll','kl']) -> str:
...
with open(
    ...
    s_list = torch.load(f, weights_only=False)['s_list']
...
for i, s in enumerate(s_list):
    ...
    for corruption in corruptions:
        ...
        for p_method in p_methods:
            ...
            kl_results[s][corruption][p_method] = []
            # load metrics for each seed folder
            for seed_folder in seed_folders:
                ...
                with open(
                    ...
                    metrics = torch.load(f, weights_only=False)
                ...
                # load predictive metrics file
                with open(
                    predictive_metrics_file(seed_folder, p_method, psi_ref, corruption), 
                    'rb'
                ) as f:
                    nll_metrics = torch.load(f, weights_only=False)
                nll_results[s][corruption][p_method].append(nll_metrics['nll'][i])
                # load kl file
                with open(
                    kl_file(seed_folder, p_method, psi_ref, corruption), 
                    'rb'
                ) as f:
                    kl_metrics = torch.load(f, weights_only=False)
                kl_results[s][corruption][p_method].append(kl_metrics['kl'][i])
...
for i, (s, markersize, delta_x) in enumerate(zip(s_list, markersizes, delta_x_list)):
    for p_method in p_methods:
        ...
        x = 1.0 * np.arange(len(corruptions)) + delta_x
        for corruption in corruptions:
            mean_values.append(np.mean(kl_results[s]
                [corruption][p_method])
            )
            std_values.append(np.std(kl_results[s]
            [corruption][p_method])
                /np.sqrt(len(kl_results[s][corruption][p_method]))
            )
        plt.errorbar(x=x, y=mean_values, yerr=std_values, color=get_color(ptype),
                    fmt=markertypes[methodclass], 
                    markersize=markersize,
    ...
    plt.yscale('log')
    plt.tight_layout()
plt.savefig(plot_file('kl'), bbox_inches='tight')
